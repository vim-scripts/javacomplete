...
class Reflection {
    ...
    public static String existedAndRead(String fqns) {
        ...
        for (StringTokenizer st = new StringTokenizer(fqns, ","); st.hasMoreTokens(); ) {
            ...
            try {
                ...
                if (!found) putPackageInfo(mapPackages, fqn);
            }
        }
    ...
    private static String getPackageList(String fqn) {
        ...
        return mapPackages.size() > 0 ? mapPackages.get(fqn)
            .toString() : "";
    }
    ...
    private static Hashtable collectClassPath() {
        ...
        if ("Kaffe".equals(System.getProperty("java.vm.name"))) {
            ...
            if (new File(System.getProperty("sun.boot.class.path")).exists()) htClasspath.put(System.getProperty("sun.boot.class.path"), "");
        }
        ...
        while (st.hasMoreTokens()) {
            ...
            if (path.endsWith(".jar") || path.endsWith(".zip")) htClasspath.put(f.toString(), "");
            else {
    ...
    public static void appendListFromJar(Hashtable subpackages, Hashtable classes, String path, String prefix) {
        try {
            for (Enumeration entries = new ZipFile(path).entries(); entries.hasMoreElements(); ) {
                ...
                if (entry.endsWith(".class") && entry.indexOf('$') == -1 && entry.startsWith(prefix)) {
                ...
                for (int i = 0; i < descents.length; i++) {
                    if (descents[i].indexOf('$') == -1) {
                        if (descents[i].endsWith(".class")) {
                            ...
                            if (!classes.containsKey(shortname)) classes.put(shortname, "");
                        } else if ((new File(fullPath + "/" + descents[i])).isDirectory()) {
                            if (!subpackages.containsKey(descents[i])) subpackages.put(descents[i], "");
                        }
                    }
                }
            }
    ...
    public static String getPackageList() {
        ...
        for (Enumeration e = collectClassPath().keys(); e.hasMoreElements(); ) {
            String path = (String) e.nextElement();
            if (path.endsWith(".jar") || path.endsWith(".zip")) appendListFromJar(path, map);
        }
        ...
        for (Enumeration e = map.keys(); e.hasMoreElements(); ) {
            ...
            sb.append("'")
                ...
                .append("':")
                .append("{'tag':'PACKAGE'");
            if (sbs[INDEX_PACKAGE].length() > 0) sb.append(",'subpackages':[")
                .append(sbs[INDEX_PACKAGE])
                .append("]");
            if (sbs[INDEX_CLASS].length() > 0) sb.append(",'classes':[")
                .append(sbs[INDEX_CLASS])
                .append("]");
            sb.append("},");
        }
        sb.append("}");
    ...
    public static void putItem(Hashtable map, String parent, String child, int index) {
        ...
        if (sbs[index].toString()
            .indexOf("'" + child + "',") == -1) sbs[index].append("'")
            .append(child)
            .append("',");
        map.put(parent, sbs);
    }
    ...
    private static void putClassInfo(Hashtable map, Class clazz) {
        if (map.containsKey(clazz.getName())) return;
        ...
        try {
            ...
            for (int i = 0, n = fields.length; i < n; i++) {
                ...
                sb.append(KEY_NAME)
                    ...
                    .append(f.getName())
                    .append("',");
                if (!f.getDeclaringClass()
                    .getName()
                    .equals(clazz.getName())) sb.append(KEY_DECLARING_CLASS)
                    .append("'")
    ...
    static void output(String s) {
        if (!debug_mode) System.out.print(s);
    }
    ...
    public static void main(String[] args) {
        ...
        if (wholeClassInfo) output(getClassInfo(className));
        else if ((option & RETURN_ALL_PACKAGE_INFO) == RETURN_ALL_PACKAGE_INFO) output(getPackageList());
